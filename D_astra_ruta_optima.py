...
def get_mesh_spacing(default_mesh=30):
    try:
        ...
        PASO_CAMINO_DEFAULT = max(30,PASO_MALLA_DEFAULT/5)
...
def construir_grafo(puntos, index_map, paso_malla):
    ...
    movs = [  (paso_malla, 0),  # Este
        (-paso_malla, 0),  # Oeste
        (0, paso_malla),  # Norte
        (0, -paso_malla),  # Sur
        (paso_malla, paso_malla),  # NE
        (paso_malla, -paso_malla),  # SE
        (-paso_malla, paso_malla),  # NO
        (-paso_malla, -paso_malla),  # SO
    ]
...
def ruta_optima_entre_nodos(nodo1: Dict, nodo2: Dict, zonas_restringidas,
                            pols, DXF_FOLDER,FOLDER_NAME_1: str,
                            raster_path: str | None = None,
                            paso_malla: float = PASO_MALLA_DEFAULT,
                            paso_camino: float = PASO_CAMINO_DEFAULT,
                            buffer_cuadricula: float = BUFFER_CUADRICULA):
    paso_malla_auxiliar=paso_malla
    buffer_cuadricula_auxiliar=buffer_cuadricula

    dist_buffer = math.hypot(nodo1['x'] - nodo2['x'], nodo1['y'] -nodo2['y'])

    if dist_buffer <=100:
        paso_malla = 10
        buffer_cuadricula=150
    elif dist_buffer <= 300 and dist_buffer > 100:
        paso_malla = 20
        buffer_cuadricula=250
    elif dist_buffer <= 1000 and dist_buffer > 300:
        paso_malla = paso_malla_auxiliar
        buffer_cuadricula=buffer_cuadricula_auxiliar*.5
    elif dist_buffer <= 2000 and dist_buffer > 1000:
        paso_malla = paso_malla_auxiliar*1.5
        buffer_cuadricula=buffer_cuadricula_auxiliar*.25
    else:
        paso_malla=paso_malla_auxiliar*4
        buffer_cuadricula=buffer_cuadricula_auxiliar*0.5
    # 1) DEM explícito
    init_dem(raster_path)
    print(paso_malla,'paso malla')
    print(dist_buffer,'buffer')
    ...
    n1, n2 = aplicar_buffer_entre_nodos(nodo1, nodo2, nodos_wtg,
                                        distancia_buffer=15, desplazamiento_adicional=15)
    ...
    if coste_total >7500:
        coste_total=distancia_final*1000000
